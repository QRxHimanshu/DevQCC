...
# from cutqc.main import CutQC as DevQcc
...
class SamplerQNN(NeuralNetwork):
        ...
        *,
        ...
        cutqc = CutQC(
            ...
            cutter_constraints={
                ...
                "max_cuts": 20,
                "num_subcircuits": [2, 3, 4, 5, 6, 7, 8, 9, 10],
            },
            verbose=True,
        )
        # devqcc = DevQcc(
        ...
        #     circuit=circuit,
        #     cutter_constraints={
        #         "max_subcircuit_width": min(7,circuit.num_qubits),
        #         "max_subcircuit_cuts": 20,
        #         "subcircuit_size_imbalance": 3,
        #         "max_cuts": 20,
        #         "num_subcircuits": [2, 3, 4, 5, 6, 7, 8, 9, 10],
        ...
        # devqcc.cut()
        cutqc.cut()
        self.mip_model = cutqc.mip_model
    ...
    def convert_to_quasi_distribution(self, probabilities):
        """Converts a list of probabilities to a QuasiDistribution object."""
        # Create a dictionary from the list of probabilities
        prob_dict = {state: prob for state, prob in enumerate(probabilities)}
        quasi_distribution = QuasiDistribution(prob_dict)
        return quasi_distribution

    def prepare_sampler_result(self,probabilities_list):
        """Converts a list of probabilities to the format expected by the Sampler."""
        quasis = [self.convert_to_quasi_distribution(prob) for prob in probabilities_list]
        metadata = [{} for _ in range(len(probabilities_list))]  # Prepare empty metadata
    ...
    def _cutforward(
        ...
        for i in range(num_samples):
            ...
            circuit = self._org_circuit.copy()
            params = circuit.parameters

            if len(parameter_values[i]) != len(params):
                raise QiskitError(
                    f"The number of values ({len(parameter_values[i])}) does not match "
                    f"the number of parameters ({len(params)})."
                )
            # print(params)
            # print(parameter_values[i])
            circuit.assign_parameters(parameter_values[i], inplace=True)
            ...
            print("-------------------------------------------")
            print("Loading cuts")
            devqcc.cut_load_mip(mip_model=self.mip_model)
            # for sc in devqcc.subcircuits:
            #     print("printing")
            #     print(sc)
            #     print("Done printing")
            print("Done loading cuts")
            devqcc.evaluate(eval_mode="sv", num_shots_fn=None)
            devqcc.build(mem_limit=32, recursion_depth=1)
            devqcc.verify()
            print(devqcc.approximation_error,devqcc.chi2_dist)

            prob = devqcc.reconstructed_prob
            prob_list.append(prob)
            devqcc.clean_data()
        
        results = self.prepare_sampler_result(prob_list)
