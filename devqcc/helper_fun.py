...
def check_valid(circuit):
    """
    If the input circuit is not fully connected, it does not need devqcc to be split into smaller circuits.
    devqcc hence only cuts a circuit if it is fully connected.
    Furthermore, devqcc only supports 2-qubit gates.
    """
    ...
    for op_node in dag.topological_op_nodes():
        if len(op_node.qargs) > 2:
            raise ValueError("devqcc currently does not support >2-qubit gates")
