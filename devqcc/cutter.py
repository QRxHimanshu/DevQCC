...
from devqcc.mapping import *
from devqcc.post_process_helper import count_subcircuit_entries, generate_compute_graph, generate_subcircuit_entries
...
class MIP_Model(object):
    ...
    def check_graph(self, n_vertices, edges):
        ...
        # print("Vertices:", set([i for (i, _) in edges]) | set([i for (_, i) in edges]))
        # print("Expected Range:", set(range(n_vertices)))
    ...
    def solve(self):
        ...
        try:
            ...
            self.model.setParam('PoolSearchMode', 2)# search mode 0 --> one solution, 1 --> multiple but non-optimal, 2 --> multiple and optimal
            self.model.setParam('PoolSolutions', 10)#num solutions
            self.model.setParam('PoolGap', 0.1)
            self.model.optimize()

            # Set the pool search mode to 2 to find the n best solutions
            
        except (gp.GurobiError, AttributeError, Exception) as e:
            print("Caught: " + e.message)
        
        if self.model.solcount > 0:
            all_solutions = []  # To store information about all solutions
            optidx = 0
            totsum = 0 # total noise score of all sub-circuits
            totiter = 1000000000
            device_map = []
            for sol_index in range(self.model.SolCount):
                self.model.Params.SolutionNumber = sol_index

                # Temporary storage for current solution's subcircuits and cut edges
                subcircuits_temp = []
                cut_edges_temp = []
                ...
                for i in range(self.num_subcircuit):
                    subcircuit = []
                    for j in range(self.n_vertices):
                        # Using Xn to get the variable value in the current solution
                        if self.vertex_var[i][j].Xn > 0.5:  # Adjust threshold as needed
                            subcircuit.append(self.id_vertices[j])
                    subcircuits_temp.append(subcircuit)

                cut_edges_idx = []
                for i in range(self.num_subcircuit):
                    for j in range(self.n_edges):
                        if self.edge_var[i][j].Xn > 0.5 and j not in cut_edges_idx:  # Adjust threshold as needed
                            cut_edges_idx.append(j)
                            u, v = self.edges[j]
                            cut_edges_temp.append((self.id_vertices[u], self.id_vertices[v]))
                circuit = self.circuit
                positions = cuts_parser(cut_edges_temp, circuit)
                subcircuits, complete_path_map = subcircuits_parser(
                    subcircuit_gates=subcircuits_temp, circuit=circuit
                )
                ...
                print("generating compute graph for solution ",sol_index)
                compute_graph = generate_compute_graph(
                    counter=counter,
                    subcircuits=subcircuits,
                    complete_path_map=complete_path_map,
                )
                print("Generation done, now counting entries")
                tempiter = count_subcircuit_entries(compute_graph)
                print("generation done")
                #tov, optlist = find_optimal_backend_matches(subcircuits, self.backends)
                #tov, optlist = find_worst_backend_matches(subcircuits, self.backends)
                tov, optlist = find_distributed_backend_matches(subcircuits, self.backends)                
                if tempiter < totiter:
                    totiter = tempiter
                    totsum = tov
                    optidx = sol_index
                    device_map = optlist
                elif tempiter == totiter:
                    if(tov > totsum):
                        totsum = tov
                        optidx = sol_index
                        device_map = optlist
                ...
                print("Total iterations is: ",tempiter)
                print("Total sum is : ", tov)
                # print("-" * 20)
                # print("-" * 20)
                # backend = Fake27QPulseV1()
                # for sc in subcircuits :
                #     print(" -- ", calculate_alignment_score(sc,backend), " -- ")
                # print("-" * 20)
                # print("-" * 20)
                print_cutter_result(
            ...
            self.model.Params.SolutionNumber=optidx
            self.device_map=device_map
            print(" ----------------- ")
            print("SOLUTION INDEX IS :",self.model.Params.SolutionNumber)
            print(" ----------------- ")
            ...
            for i in range(self.num_subcircuit):
                subcircuit = []
                for j in range(self.n_vertices):
                    if abs(self.vertex_var[i][j].Xn) > 1e-4:
                        subcircuit.append(self.id_vertices[j])
            ...
            for i in range(self.num_subcircuit):
                for j in range(self.n_edges):
                    if abs(self.edge_var[i][j].Xn) > 1e-4 and j not in cut_edges_idx:
                        cut_edges_idx.append(j)
                        u, v = self.edges[j]
...
def find_cuts(
    ...
    for num_subcircuit in num_subcircuits:
        ...
        if feasible:
            ...
            cut_solution = {
                ...
                "num_cuts": len(positions),
                "counter": counter,
                "device_map": mip_model.device_map,
            }
            break
        elif verbose:
    ...
    if verbose and len(cut_solution) > 0:
        ...
        print("-" * 20, flush=True)
    return cut_solution,mip_model

def find_cuts_with_existingcuts(
    circuit,
    max_subcircuit_width,
    max_cuts,
    num_subcircuits,
