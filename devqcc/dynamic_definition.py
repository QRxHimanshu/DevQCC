...
from devqcc.evaluator import get_num_workers
from devqcc.graph_contraction import GraphContractor
from devqcc.helper_fun import add_times
from devqcc.post_process_helper import get_reconstruction_qubit_order
...
class DynamicDefinition(object):
    ...
    def initialize_dynamic_definition_schedule(self):
        ...
        # print(f"Subcircuit capacities: {subcircuit_capacities}")
        # print(f"Subcircuit active qubits: {subcircuit_active_qubits}")
        for subcircuit_idx in subcircuit_active_qubits:
            num_zoomed = 0
            num_active = subcircuit_active_qubits[subcircuit_idx]
            num_merged = (
            ...
            # print(f"Subcircuit {subcircuit_idx} initial state: {schedule['subcircuit_state'][subcircuit_idx]}")
        return schedule
    ...
    def merge_states_into_bins(self):
        ...
        for rank in range(num_workers):
            python_command = (
                "python3 -m devqcc.parallel_merge_probs --data_folder %s --rank %d --num_workers %d"
...
#     )
#     chi2_dist = chi2_distance(target=ground_truth, obs=real_probability)
#     fidel = fidelity(target=ground_truth, obs=real_probability)
#     res = absres(target=ground_truth, obs=real_probability)
#     hellfi = hellinger_fidelity(target=ground_truth, obs=real_probability)
#     klfid = klfidelity(target=ground_truth, obs=real_probability)
#     jsfid = jensen_shannon_fidelity(target=ground_truth, obs=real_probability)
#     tvdf = tvd_fidelity(target=ground_truth, obs=real_probability)
#     relent = relative_entropy(target=ground_truth, obs=real_probability)
#     correl = correlation(target=ground_truth, obs=real_probability)
#     quar1 = quartile1_hop(target=ground_truth, obs=real_probability)
#     quar2 = quartile1_hop(target=ground_truth, obs=real_probability)
#     quar3 = quartile1_hop(target=ground_truth, obs=real_probability)
#     return reconstructed_prob, approximation_error, chi2_dist, fidel, res, hellfi, klfid, jsfid, tvdf,relent,correl, quar1, quar2, quar3
...
def full_verify(full_circuit, complete_path_map, subcircuits, dd_bins):
    ...
    # correl = correlation(target=ground_truth, obs=real_probability)
    quar1 = quartile1_hop(target=ground_truth, obs=real_probability)
    quar2 = quartile2_hop(target=ground_truth, obs=real_probability)
    quar3 = quartile3_hop(target=ground_truth, obs=real_probability)
    return real_probability, approximation_error, chi2_dist, hellfi, klfid, jsfid, tvdf, relent, quar1, quar2, quar3
