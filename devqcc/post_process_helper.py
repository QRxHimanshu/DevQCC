...
def count_subcircuit_entries(compute_graph):
    """
    Count total subcircuit entries without generating them.
    
    Returns:
    - Total count of subcircuit entries.
    """
    ...
    for subcircuit_idx in compute_graph.nodes:
        ...
        # The number of combinations of bases for each edge is 4 (["I", "X", "Y", "Z"])
        num_edge_combinations = 4 ** len(subcircuit_edges)

        # Each subcircuit edge configuration will be considered as an entry
        total_entries_count += num_edge_combinations
