...
def run_subcircuit_instances(
    device_map, backends, subcircuits, subcircuit_instances, eval_mode, num_shots_fn, data_folder
):
    """
    ...
    for subcircuit_idx in subcircuit_instances:
        backend = backends[device_map[subcircuit_idx]]
        ...
        for rank in range(num_workers):
            ...
            if len(rank_jobs) > 0:
                ...
                python_command = (
                    "python3 -m devqcc.parallel_run_subcircuits --data_folder %s --backend %s --subcircuit_idx %d --rank %d"
                    % (data_folder, backend, subcircuit_idx, rank)
                )
    ...
    for subcircuit_idx in subcircuit_entries:
        ...
        for rank in range(num_workers):
            ...
            if len(rank_jobs) > 0:
                ...
                python_command = (
                    "python3 -m devqcc.parallel_attribute_shots --data_folder %s --subcircuit_idx %d --rank %d"
