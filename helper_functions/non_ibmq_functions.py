...
def evaluate_circ(circuit, backend, bname="", options=None):
    ...
    if backend == "statevector_simulator":
        ...
        print("Backend name is", bname)
        provider = FakeProvider()
        backend = provider.get_backend(name=bname)
        sim = AerSimulator.from_backend(backend)
        # simulator = Aer.get_backend(backend, max_memory_mb=max_memory_mb)
        if isinstance(options, dict) and "num_shots" in options:
            num_shots = options["num_shots"]
        else:
            num_shots = 1024# number of shots can be varied.......
        if isinstance(options, dict) and "memory" in options:
            memory = options["memory"]
        else:
            memory = False
        ...
        # result = sim.run(tcirc, shots=num_shots, memory=memory).result()
        # counts_noise = result_noise.get_counts(0)
        result = backend.run(circuit, shots=num_shots, memory=memory).result()
        # print("Ended Running")
        if memory:
            qasm_memory = np.array(result.get_memory(circuit))
            # assert len(qasm_memory) == num_shots
